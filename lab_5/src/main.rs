...
fn main() {
    let context = HashMap::from([("x", 12), ("y", 30), ("z", 0)]);
    let a = 5;
    let mut res = 0;

    let when_expr = when(constant("x"), 10, read_from(&a));
    let save_expr = save_in(&mut res, when_expr);

    let mut k = 10;
    let volatile = volatile(&mut k, "z", when(constant("z"), 15, 0));

    let mut prog = seq(
        seq(print(save_expr), nothing()).shorten_1(),
        seq(seq(nothing(), nothing()).collapse(), repeat::<5, _>(print(volatile))).shorten_2()
    );

    prog.exec_stmt(&context);
}
...
struct When<T: Expr, U: Expr, W: Expr> {
    condition: T,
    if_true: U,
    if_false: W
}
fn when<T: Expr, U: Expr, W: Expr>(condition: T, if_true: U, if_false: W) -> When<T, U, W> {
    When { condition, if_true, if_false }
}
impl<T: Expr, U: Expr, W: Expr> Expr for When<T, U, W> {
    fn exec_expr(&mut self, context: &Context) -> u64 {
        let condition_result = self.condition.exec_expr(context);

        if condition_result != 0 {
            self.if_true.exec_expr(context)
        }
        else {
            self.if_false.exec_expr(context)
        }
    }
}
...
fn read_from<'a>(source: &'a u64) -> ReadFrom<'a> {
    ReadFrom { source }
}
...
fn volatile<'a, T: Expr>(destination: &'a mut u64, name: &'static str, inner: T) -> Volatile<'a, T> {
    Volatile { destination, name, inner }
}
impl<'a, T: Expr> Expr for Volatile<'a, T> {
    fn exec_expr(&mut self, context: &Context) -> u64 {
        let mut tmp_context = context.clone();
        tmp_context.insert(self.name, *self.destination);

        *self.destination = self.inner.exec_expr(&tmp_context);
        *self.destination
    }
}
...
impl<T: Expr> Stmt for Print<T> {
    fn exec_stmt(&mut self, context: &Context) {
        let result = self.inner.exec_expr(context);
        println!("{}", result);
    }
}
